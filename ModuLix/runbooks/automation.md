...
### 1) Install local collection overlays (optional)

Use this only when developing local `ansible-collection-*` repositories.
Run this when you need local collection changes to overlay EE-provided collections.
...
1) EE-only runtime (recommended):  
No local install step needed. Collections come from the configured EE image.
